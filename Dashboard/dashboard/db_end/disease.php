 ...
 */
    ...
    class disease_encounterPage extends DetailPage
    {
    ...
    class diseasePage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
        ...
        protected function getFiltersColumns()
        {
            return array(
                new FilterColumn($this->dataset, 'id', 'id', 'Id'),
                new FilterColumn($this->dataset, 'disease_id', 'disease_id', 'Disease Id'),
                new FilterColumn($this->dataset, 'disease_name', 'disease_name', 'Disease Name')
            );
        }
        ...
        protected function setupQuickFilter(QuickFilter $quickFilter, FixedKeysArray $columns)
        {
            $quickFilter
                ->addColumn($columns['id'])
                ->addColumn($columns['disease_id'])
                ->addColumn($columns['disease_name']);
        }
        ...
        protected function setupFilterBuilder(FilterBuilder $filterBuilder, FixedKeysArray $columns)
        {
            ...
            $main_editor = new TextEdit('disease_id_edit');
            ...
            $main_editor = new TextEdit('disease_name_edit');
        ...
        protected function AddFieldColumns(Grid $grid, $withDetails = true)
        {
            if (GetCurrentUserPermissionSetForDataSource('disease.encounter')->HasViewGrant() && $withDetails)
            {
            //
            // View column for disease_encounter detail
            //
            $column = new DetailColumn(array('disease_id'), 'disease.encounter', 'disease_encounter_handler', $this->dataset, 'Encounter');
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $grid->AddViewColumn($column);
            }
            ...
            //
            // View column for disease_id field
            //
            $column = new TextViewColumn('disease_id', 'disease_id', 'Disease Id', $this->dataset);
            $column->SetOrderable(true);
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $column->SetDescription('');
            $column->SetFixedWidth(null);
            ...
            //
            $column = new TextViewColumn('disease_name', 'disease_name', 'Disease Name', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('diseaseGrid_disease_name_handler_list');
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $column->SetDescription('');
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('diseaseGrid_disease_name_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        }
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            // Edit column for disease_id field
            //
            $editor = new TextEdit('disease_id_edit');
            $editor->SetMaxLength(64);
            $editColumn = new CustomEditColumn('Disease Id', 'disease_id', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            ...
            //
            // Edit column for disease_name field
            //
            $editor = new TextEdit('disease_name_edit');
            $editor->SetMaxLength(80);
            $editColumn = new CustomEditColumn('Disease Name', 'disease_name', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        protected function AddPrintColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('diseaseGrid_disease_name_handler_print');
            $grid->AddPrintColumn($column);
        }
        ...
        protected function AddExportColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('diseaseGrid_disease_name_handler_export');
            $grid->AddExportColumn($column);
        }
        ...
        private function AddCompareColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('diseaseGrid_disease_name_handler_compare');
            $grid->AddCompareColumn($column);
        }
        ...
        protected function doRegisterHandlers() {
            $detailPage = new disease_encounterPage('disease_encounter', $this, array('disease_id'), array('disease_id'), $this->GetForeignKeyFields(), $this->CreateMasterDetailRecordGrid(), $this->dataset, GetCurrentUserPermissionSetForDataSource('disease.encounter'), 'UTF-8');
            $detailPage->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource('disease.encounter'));
            $detailPage->SetTitle('Encounter');
            $detailPage->SetMenuLabel('Encounter');
            $detailPage->SetHeader(GetPagesHeader());
            $detailPage->SetFooter(GetPagesFooter());
            $detailPage->SetHttpHandlerName('disease_encounter_handler');
            $handler = new PageHTTPHandler('disease_encounter_handler', $detailPage);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for disease_name field
            //
            $column = new TextViewColumn('disease_name', 'disease_name', 'Disease Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'diseaseGrid_disease_name_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for disease_name field
            //
            $column = new TextViewColumn('disease_name', 'disease_name', 'Disease Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'diseaseGrid_disease_name_handler_print', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for disease_name field
            //
            $column = new TextViewColumn('disease_name', 'disease_name', 'Disease Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'diseaseGrid_disease_name_handler_compare', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            //
            // View column for disease_name field
            //
            $column = new TextViewColumn('disease_name', 'disease_name', 'Disease Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'diseaseGrid_disease_name_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
        }
    ...
    try
    {
        $Page = new diseasePage("disease", "disease.php", GetCurrentUserPermissionSetForDataSource("disease"), 'UTF-8');
        $Page->SetTitle('Disease');
        $Page->SetMenuLabel('Disease');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("disease"));
