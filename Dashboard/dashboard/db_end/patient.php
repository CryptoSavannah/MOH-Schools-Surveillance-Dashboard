 ...
 */
    ...
    class patient_encounterPage extends DetailPage
    {
    ...
    class patientPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
        ...
        protected function getFiltersColumns()
        {
            return array(
                ...
                new FilterColumn($this->dataset, 'sex', 'sex', 'Sex')
            );
        }
        ...
        protected function setupFilterBuilder(FilterBuilder $filterBuilder, FixedKeysArray $columns)
        {
            ...
            $main_editor = new ComboBox('sex_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
            $main_editor->addChoice('F', 'F');
            $main_editor->addChoice('M', 'M');
            $main_editor->SetAllowNullValue(false);
            
            $multi_value_select_editor = new MultiValueSelect('sex');
            $multi_value_select_editor->setChoices($main_editor->getChoices());
            
            $text_editor = new TextEdit('sex');
        ...
        protected function AddFieldColumns(Grid $grid, $withDetails = true)
        {
            if (GetCurrentUserPermissionSetForDataSource('patient.encounter')->HasViewGrant() && $withDetails)
            {
            //
            // View column for patient_encounter detail
            //
            $column = new DetailColumn(array('patient_id'), 'patient.encounter', 'patient_encounter_handler', $this->dataset, 'Encounter');
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $grid->AddViewColumn($column);
            }
            ...
            //
            $column = new TextViewColumn('sex', 'sex', 'Sex', $this->dataset);
            $column->SetOrderable(true);
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $column->SetDescription('');
            $column->SetFixedWidth(null);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new ComboBox('sex_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
        ...
        protected function doRegisterHandlers() {
            $detailPage = new patient_encounterPage('patient_encounter', $this, array('patient_id'), array('patient_id'), $this->GetForeignKeyFields(), $this->CreateMasterDetailRecordGrid(), $this->dataset, GetCurrentUserPermissionSetForDataSource('patient.encounter'), 'UTF-8');
            $detailPage->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource('patient.encounter'));
            $detailPage->SetTitle('Encounter');
            $detailPage->SetMenuLabel('Encounter');
            $detailPage->SetHeader(GetPagesHeader());
            $detailPage->SetFooter(GetPagesFooter());
            $detailPage->SetHttpHandlerName('patient_encounter_handler');
            $handler = new PageHTTPHandler('patient_encounter_handler', $detailPage);
            GetApplication()->RegisterHTTPHandler($handler);
        }
    ...
    try
    {
        $Page = new patientPage("patient", "patient.php", GetCurrentUserPermissionSetForDataSource("patient"), 'UTF-8');
        $Page->SetTitle('Patient');
        $Page->SetMenuLabel('Patient');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("patient"));
